...
CircularBuffer<float> DriveTrain::AccelerometerBuffer(DriveTrain::kBufferSize);
std::shared_ptr<BuiltInAccelerometer> DriveTrain::RoboAccels;
float DriveTrain::Accel_Value_x;
float DriveTrain::Accel_Value_y;
float DriveTrain::Accel_Value_z;
float DriveTrain::Accel_Center_x;
float DriveTrain::Accel_Center_y;
float DriveTrain::Accel_Center_z;
float DriveTrain::accel_angle_y;

DriveTrain::DriveTrain() : Subsystem("DriveTrain"),
	AccelService(DriveTrain::UpdateChassisAngle) {
    ...
    AccelService.StartPeriodic(0.050);
}
...
//	accel_angle_x = atan(result);
	...
	DriveTrain::AccelerometerBuffer.PushFront(accel_angle_y);
	for (j = 0; j < kBufferSize; j++) {
		AverageAngle += AccelerometerBuffer[j];
	}
	AverageAngle /= (float)kBufferSize;
	accel_angle_y =  AverageAngle;
...
float DriveTrain::GetChassisAngle() {
	return DriveTrain::accel_angle_y * 360.0 / (M_PI * 2.0);
}
