...
DriveTrain::DriveTrain() : Subsystem("DriveTrain"),
	kDefaultVoltageRamp(prefs->GetDouble("VoltageRamp", 27.0)),
			AccelBufferX(kBufferSize),
	AccelBufferY(kBufferSize), AccelBufferZ(kBufferSize) {
...
void DriveTrain::TankDriveWithTriggers(float Left, float Right, float Trigger) {
	float newLeft = 0.0;
 	...
 	ProcessedLeft = DEADBAND(AxisPower(ProcessedLeft, fExponent), 0.1);

 	ProcessedRight = DEADBAND(AxisPower(ProcessedRight, fExponent), 0.1);
...
void DriveTrain::UpdateChassisPitch() {
	...
	AccelBufferX.PushBack(RoboAccels->GetX());
	AccelBufferY.PushBack(RoboAccels->GetY());
	AccelBufferZ.PushBack(RoboAccels->GetZ());
	...
	AvgX /= (float)kBufferSize;
	AvgY /= (float)kBufferSize;
	AvgZ /= (float)kBufferSize;
